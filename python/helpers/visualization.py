...
def generate_activity_graph(months: list[dict], graphs_dir: Path, data: dict) -> None:
    ...
    for year in sorted(data["yearly_stats"].keys()):
        ...
        ax.text(
            idx + year_count / 2 - 0.5,
            year_avg + data["max_size"] * 0.03,
            f"{year}: {year_avg:.1f}",
            ha="center",
            fontsize=9,
            fontweight="bold",
...
def generate_dashboard(months: list[dict], graphs_dir: Path, data: dict) -> None:
    ...
    if overall_moods:
        ...
        ax3.legend(
            wedges,
            [m.capitalize() for m in mood_names],
            loc="center left",
    ...
    ax5.fill_between(
        ...
        alpha=0.2,
        color="#e74c3c",
        label=f"+/- 1 Std Dev ({data['std_size']:.1f})",
    )
    ...
    ax6_twin = ax6.twinx()
    bars2 = ax6_twin.bar(
        x_positions + width / 2, year_avgs, width, label="Avg/Month", color="#2ecc71"
    )
    ...
    ax6.set_title(
        "Year-over-Year: Total Scrobbles vs Average per Month",
        fontsize=12,
        fontweight="bold",
    )
    ...
    plt.suptitle(
        "Last.fm Scrobble Analysis - Summary Dashboard",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
...
def generate_graphs(months: list[dict], options: GraphOptions | None = None) -> None:
    ...
    graph_generators: list[tuple[bool, Callable[..., None], tuple[Any, ...]]] = [
        ...
        (
            options.mood_timeline,
            generate_mood_timeline_graph,
            (months, graphs_dir, data),
        ),
